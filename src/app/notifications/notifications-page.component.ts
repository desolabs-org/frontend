...
export class NotificationsPageComponent implements OnInit {
  ...
  getPage(page: number) {
    ...
    return this.backendApi
      .GetNotifications(
        ...
        NotificationsPageComponent.PAGE_SIZE /*NumToFetch*/
      )
      .toPromise()
      .then(
        (res) => {
          ...
          this.pagedIndexes[page + 1] =
            res.Notifications[res.Notifications.length - 1]?.Index - 1 || 0;

          // if the chunk was incomplete or the Index was zero we're done
          if (
            chunk.length < NotificationsPageComponent.PAGE_SIZE ||
            this.pagedIndexes[page + 1] === 0
          ) {
            this.lastPage = page;
          }
  ...
  protected transformNotification(notification: any) {
    ...
    if (txnMeta.TxnType === 'BASIC_TRANSFER') {
      ...
      if (ccMeta.OperationType === 'buy') {
        ...
        result.action = `${actorName} sold <b>${this.globalVars.nanosToDeSo(
          ccMeta.CreatorCoinToSellNanos
        )} $${userProfile.Username}.</b>`;
        return result;
      }
      ...
      result.icon = likeMeta.IsUnlike
        ? 'fas fa-heart-broken fc-red'
        : 'fas fa-heart fc-red';
      ...
      if (
        ...
        } for ${this.globalVars.nanosToDeSo(
        ...
        result.icon = nftBidMeta.BidAmountNanos
          ? 'fas fa-dollar-sign fc-blue'
          : 'fas fa-dollar-sign fc-red';
        return result;
      } else if (
        this.globalVars.loggedInUser?.PublicKeyBase58Check ===
        ...
        const additionalCoinRoyaltiesMap: { [k: string]: number } =
          nftBidMeta.AdditionalCoinRoyaltiesMap || {};
        const additionalDESORoyaltiesMap: { [k: string]: number } =
          nftBidMeta.AdditionalDESORoyaltiesMap || {};
        if (
          this.globalVars.loggedInUser?.PublicKeyBase58Check in
    ...
    await this.datasource.adapter.update({
      predicate: ({ $index, data, element }) => {
        ...
        if ($index === index) {
          ...
          currentNotification.post.Comments =
            currentNotification.post.Comments || [];
          currentNotification.post.Comments.unshift(_.cloneDeep(newComment));
          currentNotification.post = this.globalVars.incrementCommentCount(
            currentNotification.post
          );
          return [currentNotification];
        } else if (
          currentNotification.post &&
          this.globalVars.getPostContentHashHex(currentNotification.post) ===
            uiPostParentHashHex
        ) {
  ...
  getRoyaltyString(
    ...
    const desoRoyaltyStr = desoRoyalty
      ? `a royalty of ${
          usePercent
            ? this.getPercentRoyaltyString(desoRoyalty)
            : this.getRoyaltyAmountString(desoRoyalty)
        } to your wallet`
      : '';
    if (!coinRoyaltyStr && !desoRoyaltyStr) {
      return '';
    }
    return `${desoRoyaltyStr}${
  ...
  getRoyaltyAmountString(royalty: number | undefined): string {
    ...
    return `${this.globalVars.nanosToDeSo(
      royalty
    )} DESO (~${this.globalVars.nanosToUSD(royalty)})`;
  }

  infiniteScroller: InfiniteScroller = new InfiniteScroller(
    NotificationsPageComponent.PAGE_SIZE,
    this.getPage.bind(this),
    NotificationsPageComponent.WINDOW_VIEWPORT,
    NotificationsPageComponent.BUFFER_SIZE
  );
  datasource: IDatasource<
    IAdapter<PostEntryResponse>
