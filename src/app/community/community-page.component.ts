...
import { CanPublicKeyFollowTargetPublicKeyHelper } from 'src/lib/helpers/follows/can_public_key_follow_target_public_key_helper';
...
export class CommunityPageComponent implements OnDestroy {
  ...
  getPage(page: number) {
    ...
    return this.backendApi
      .GetFollows(
        ...
        fetchPubKey,
        CommunityPageComponent.PAGE_SIZE
      )
      .toPromise()
      .then((res) => {
        ...
        if (
          sortedProfileEntries.length < CommunityPageComponent.PAGE_SIZE ||
          this.pagedKeys[page + 1] === ''
        ) {
          this.lastPage = page;
  ...
  _handleTabClick(tabName) {
    let routeSuffix;
    if (tabName === CommunityPageComponent.FOLLOWING) {
      routeSuffix = RouteNames.FOLLOWING;
    } else if (tabName === CommunityPageComponent.FOLLOWERS) {
  ...
  _setStateFromActivatedRoute(route) {
    ...
    switch (path) {
      case this.appData.RouteNames.FOLLOWING: {
        ...
        this.activeTab = CommunityPageComponent.FOLLOWING;
        this.lastPage = null;
      ...
      case this.appData.RouteNames.FOLLOWERS: {
        ...
        this.activeTab = CommunityPageComponent.FOLLOWERS;
        this.lastPage = null;
        this.infiniteScroller.reset();
        this.datasource.adapter.reset();
        break;
    ...
    this.loggedInUserSubscription = this.appData.loggedInUserObservable.subscribe(
      (loggedInUserObservableResult) => {
        ...
        if (!loggedInUserObservableResult.isSameUserAsBefore) {
          this.datasource.get(0, CommunityPageComponent.PAGE_SIZE, null);
        }
      }
    );
  ...
  infiniteScroller: InfiniteScroller = new InfiniteScroller(
    ...
    CommunityPageComponent.WINDOW_VIEWPORT,
    CommunityPageComponent.BUFFER_SIZE
  );
