...
export class UpdateProfilePageComponent implements OnInit, OnChanges {
  ...
  _setProfileErrors(): boolean {
    ...
    if (this.descriptionInput.length > 1024) {
    ...
    this._callBackendUpdateProfile().subscribe(
      ...
      (err) => {
        ...
        SwalHelper.fire({
          ...
          confirmButtonText: lowBalance ? 'Buy ÐESO' : null,
  ...
  _updateProfileSuccess(comp: UpdateProfilePageComponent) {
  ...
  _updateProfileFailure(comp: UpdateProfilePageComponent) {
