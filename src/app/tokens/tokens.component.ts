...
import { TokenBurnModalComponent } from './burn-modal/token-burn-modal.component';
...
export class TokensComponent implements OnInit, OnDestroy {
  ...
  tabs = [TokensComponent.myDAOTab, TokensComponent.daoCoinsTab];
  activeTab: string = TokensComponent.myDAOTab;
  ...
  ngOnInit() {
    ...
    if (this.globalVars.loggedInUser?.ProfileEntryResponse) {
      ...
      this.activeTab = TokensComponent.daoCoinsTab;
      this.tabs = [TokensComponent.daoCoinsTab];
    }
  ...
  _handleTabClick(tab: string) {
    this.showDAOCoinHoldings = tab === TokensComponent.daoCoinsTab;
    this.lastPage = Math.floor(
      (this.showDAOCoinHoldings ? this.daoCoinHoldings : this.myDAOCapTable)
        .length / TokensComponent.PAGE_SIZE
    );
    this.activeTab = tab;
    this.scrollerReset();
  }
  ...
  infiniteScroller: InfiniteScroller = new InfiniteScroller(
    TokensComponent.PAGE_SIZE,
    this.getPage.bind(this),
    TokensComponent.WINDOW_VIEWPORT,
    TokensComponent.BUFFER_SIZE,
    TokensComponent.PADDING
  );
  ...
  getPage(page: number) {
    ...
    const startIdx = page * TokensComponent.PAGE_SIZE;
    const endIdx = (page + 1) * TokensComponent.PAGE_SIZE;
