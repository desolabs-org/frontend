...
export class MessagesInboxComponent implements OnInit, OnChanges {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      this.activeTab =
        params.messagesTab &&
        params.messagesTab in MessagesInboxComponent.QUERYTOTAB
  ...
  ngOnChanges(changes: any) {
    ...
    if (
      changes.messageThreads.previousValue === null &&
      changes.messageThreads.currentValue.length > 0
    ) {
  ...
  _setSelectedThreadBasedOnDefaultThread() {
    ...
    let interval = setInterval(() => {
      ...
      if (this.defaultContactUsername || this.defaultContactPublicKey) {
        defaultThread = _.find(
          orderedContactsWithMessages,
          (messageContactResponse) => {
            let responseUsername =
              messageContactResponse.ProfileEntryResponse?.Username;
            let matchesUsername =
              responseUsername && responseUsername === this.contactUsername;
            let matchesPublicKey =
              this.contactUsername ===
              messageContactResponse.PublicKeyBase58Check;
    ...
    this.backendApi
      ...
      .subscribe(
        ...
        (err) => {
          ...
          this.globalVars.logEvent('user : all-message-read : error', {
            parsedError,
          });
  ...
  _handleCreatorSelectedInSearch(creator: any) {
    ...
    let existingThread = this._getThreadWithPubKey(
      ...
      this.backendApi
        ...
        .subscribe(
          ...
          (err) => {
            console.log(err);
            const parsedError = this.backendApi.stringifyError(err);
            this.globalVars.logEvent('user : message-read : error', {
              parsedError,
            });
            this.globalVars._alertError(parsedError);
          }
